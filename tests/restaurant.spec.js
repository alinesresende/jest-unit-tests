...
describe('10 - Implemente a função `createMenu`, bem como seus casos de teste', () => {
  it('Verifica se a função `createMenu` tem o comportamento esperado', () => {
    const objetoRetornado = createMenu({ food: {pizza: 15.00}, drink: {coca: 5.00} });
    // fail('Teste vazio!');
    ...
    expect(objetoRetornado.fetchMenu()).toEqual({ food: {pizza: 15.00}, drink: {coca: 5.00} });
    ...
    expect(objetoRetornado.order('coca')).toEqual(['coca']);
    expect(objetoRetornado.order('pao')).toBe('Item indisponível');
    ...
    expect(objetoRetornado.consumption).toHaveLength(3);
    // 10: Verifique se a função `order` aceita que pedidos repetidos sejam acrescidos a `consumption`.
    expect(objetoRetornado.consumption[0]).toBe('coca')
    expect(objetoRetornado.consumption[2]).toBe('coca')
    ...
    expect(objetoRetornado.pay()).toBe(27.5)
